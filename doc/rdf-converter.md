...
You will notice that initially each entry is displayed using the preferred label for every language. You can change your language preferences by clicking on the [globe](https://showvoc.uniroma2.it/doc/user/data_view.jsf#the_data_structure_view) icon. Once you have done this, click the refresh icon to re-load just the languages you want. You can also choose to render by the canonical id of the entry by toggling the "A" button.

You will notice that the canonical ids we are using do not include the language prefix. The decision for this is documented [here](decisions/ontologies/url-naming.md). This results in a few duplicates in some of the taxonomies which we hope to address in the data.
...
Each taxonomy is stored as a Concept Scheme. Most Datasets will only show one concept scheme, except for origins and food_ingredients, which combine all of the related taxonomies that contribute to overall hierarchy.
...
You may find that some property names indicate incorrect spelling, e.g. [ciqualFoodProxyCode](https://taxonomies.openfoodfacts.net/showvoc/#/datasets/food_categories/1.0.0/data?resId=https:%2F%2Fopenfoodfacts.org%2Fdata%2Ftaxonomies%2Fcore%23ciqualFoodProxyCode) instead of `ciqualProxyFoodCode`. You can find these rogue entries by simply searching the original taxonomy `.txt` file (converting back to snake_case) or you could use the [SPARQL](https://taxonomies.openfoodfacts.net/showvoc/#/datasets/food_categories/1.0.0/sparql) tab to run a query on the Dataset, e.g. 
...
ShowVoc does not behave like a normal browser when it queries Wikidata entries and Wikidata currently rejects these requests as part of its routine protection measures, resulting in an error like `Server returned HTTP response code: 403 for URL:`. You can manually see the external references by copying the URL and pasting it into a new browser tab.

In the case of Agribalyse and CIQUAL the data is not published as a true OWL ontology, so you also have to manually copy and paste the URL to see the relevant web page.
