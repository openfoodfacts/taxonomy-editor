...
TAXONOMY_JSON_ROOT = os.environ.get(
    "TAXONOMY_JSON_ROOT", "https://static.openfoodfacts.org/data/taxonomies"
)
...
def getTaxonomyJson(name):
    return json.loads(request.urlopen(f"{TAXONOMY_JSON_ROOT}/{name}.json").read().decode("utf-8"))
